...
_DEFAULT_BLOCKS: dict[str, str] = {
    ...
    "dataset_guidance": (
        ...
        "- In these cases, explain WHY synthetic data is appropriate.\n"
    ),
...
_DEFAULT_SUB_PROMPTS: dict[str, dict[str, Any]] = {
    ...
    "architecture_planning": {
        ...
        "user": (
            ...
            "7. CRITICAL: Child classes MUST NOT be thin wrappers that only "
            "change __init__ parameters. Each child class MUST override at "
            "least one core method with DIFFERENT implementation logic. "
            "For example, if comparing attention mechanisms, each class must "
            "implement its own attention computation, not just change num_heads. "
            "If the differentiator is only a hyperparameter, the child class "
            "must STILL implement the key method to demonstrate the difference "
            "in behavior (e.g., different forward pass, different loss, "
            "different training loop).\n"
            "8. Design child classes as STRATEGY variants, not PARAMETER variants. "
            "Each variant should embody a fundamentally different approach, "
            "with distinct code paths.\n"
            "9. NEVER override nn.Module reserved methods (train, eval, zero_grad) "
            "with different signatures. If you need a custom train method, name it "
            "`run_training()` or `fit()` instead. Overriding `train()` with extra "
            "required arguments will break `model.eval()` which calls `train(False)`.\n\n"
            "## Output Format\n"
